...
function iconHTML(m,cls='mode-icon-img'){
 ...
 if(!url) return `<span class="icon-fallback">${esc(m.i||'')}</span>`;
 return `<img class="${cls}" src="${esc(url)}" alt="${esc(m.n)}" loading="eager" onerror="this.style.display='none';this.nextElementSibling.style.display='grid'"><span class="icon-fallback" style="display:none">${esc(m.i||'')}</span>`;
}
