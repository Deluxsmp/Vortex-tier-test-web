...
function emojiFor(pos){return pos===1?'ð':pos<=3?'ð':'âï¸'}
...
function renderMode(q){
 ...
 boardEl.innerHTML=`<div class="mode-board">${[1,2,3,4,5].map(n=>`<div class="tier-column tier-${n}"><div class="tier-head"><span>ð</span>Tier ${n}</div><div class="tier-list">${filtered[n].length?filtered[n].map(p=>`<div class="tier-player" data-id="${p.id}"><span class="tier-skin-frame"><img src="${esc(skinUrl(p))}" alt="${esc(p.name)}" onerror="skinFallback(this)"></span><span class="name">${esc(p.name)}</span><span class="up">â</span></div>`).join(''):'<div class="tier-player"><span class="name">No players</span></div>'}</div></div>`).join('')}</div>`;
}
function modalFor(p){
 ...
 modalBody.innerHTML=`<div class="modal-player-top"><div class="modal-avatar"><img src="${esc(skinUrl(p))}" alt="${esc(p.name)}" onerror="skinFallback(this)"></div><h2>${esc(p.name)}</h2><div class="modal-rank">${emojiFor(p.position)} ${title}</div><div class="modal-region-text">${esc(regionName)}</div><a class="namemc-link" href="https://namemc.com/profile/${encodeURIComponent(p.name)}" target="_blank" rel="noopener">â§ NameMC â</a></div><div class="modal-section-title">Position</div><div class="modal-position-box"><span class="modal-position-number">${p.position}.</span><span>ð <strong>OVERALL</strong> <small>(${p.totalPoints} points)</small></span></div><div class="modal-section-title">Tiers</div><div class="modal-tiers-grid">${tiers}</div>`;
