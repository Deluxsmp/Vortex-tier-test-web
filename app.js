const VORTEX_DATA={"modes":[{"id":"overall","n":"Overall","i":"🏆"},{"id":"vanilla","n":"Vanilla","i":"🟪"},{"id":"uhc","n":"UHC","i":"❤️"},{"id":"pot","n":"Pot","i":"🧪"},{"id":"nethop","n":"NetHop","i":"🟣"},{"id":"smp","n":"SMP","i":"🟢"},{"id":"sword","n":"Sword","i":"🗡️"},{"id":"axe","n":"Axe","i":"🪓"},{"id":"mace","n":"Mace","i":"🔨"}],"icons":{},"site":{"siteName":"Vortex Tier","discordUrl":"https://discord.com","serverIp":"mc.vortextier.net"},"players":[]};
let DATA=VORTEX_DATA;
DATA.players=(typeof VORTEX_PLAYERS!=="undefined"&&Array.isArray(VORTEX_PLAYERS))?VORTEX_PLAYERS:[];
...
DATA.players.sort((a,b)=>(Number(b.totalPoints)||0)-(Number(a.totalPoints)||0));
DATA.players.forEach((p,i)=>p.position=i+1);
...
function modeById(id){return DATA.modes.find(m=>m.id===id)||{id:id,n:id,i:'â€¢'}}
...
function emojiFor(pos){return pos===1?'ðŸ‘‘':pos<=3?'ðŸ…':'âš”ï¸'}
...
function renderMode(q){
 ...
 boardEl.innerHTML=`<div class="mode-board">${[1,2,3,4,5].map(n=>`<div class="tier-column tier-${n}"><div class="tier-head"><span>ðŸ†</span>Tier ${n}</div><div class="tier-list">${filtered[n].length?filtered[n].map(p=>`<div class="tier-player" data-id="${p.id}"><span class="tier-skin-frame"><img src="${esc(skinUrl(p))}" alt="${esc(p.name)}" onerror="skinFallback(this)"></span><span class="name">${esc(p.name)}</span><span class="up">âŒƒ</span></div>`).join(''):'<div class="tier-player"><span class="name">No players</span></div>'}</div></div>`).join('')}</div>`;
}
function modalFor(p){
 ...
 modalBody.innerHTML=`<div class="modal-player-top"><div class="modal-avatar"><img src="${esc(skinUrl(p))}" alt="${esc(p.name)}" onerror="skinFallback(this)"></div><h2>${esc(p.name)}</h2><div class="modal-rank">${emojiFor(p.position)} ${title}</div><div class="modal-region-text">${esc(regionName)}</div><a class="namemc-link" href="https://namemc.com/profile/${encodeURIComponent(p.name)}" target="_blank" rel="noopener">â—§ NameMC â†—</a></div><div class="modal-section-title">Position</div><div class="modal-position-box"><span class="modal-position-number">${p.position}.</span><span>ðŸ† <strong>OVERALL</strong> <small>(${p.totalPoints} points)</small></span></div><div class="modal-section-title">Tiers</div><div class="modal-tiers-grid">${tiers}</div>`;
...
if(DATA.site?.siteName)document.title=DATA.site.siteName+' â€” Rankings'; const server=document.querySelector('.server'); if(server&&DATA.site?.serverIp) server.innerHTML='<i></i> '+esc(DATA.site.serverIp); renderModes();render();
